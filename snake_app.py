...
args = sys.argv;
number = args[1];
setting =importlib.import_module("settings"  + str(args[1]),'settings');
settings = setting.settings;
a = open("populations/Change" + str(number) + "/data.txt",'w');
a.close();
    ...
    def update(self) -> None:
        ...
        # Current individual is dead         
            ...
            if (self.current_generation > 0 and self._current_individual == self._next_gen_size) or\
                ...
                save_snake('populations/Change' + str(number),"BestSnake" + str(self.current_generation),self.population.fittest_individual,self.settings)
                f = open('populations/Change' + str(number) + "/data.txt",'a')
                f.write(str(self.population.fittest_individual.fitness))
                f.write(' ')
                f.write(str(self.population.fittest_individual.score))
                f.write('\n')
                f.close()
                if (self.current_generation == 200): quit()
